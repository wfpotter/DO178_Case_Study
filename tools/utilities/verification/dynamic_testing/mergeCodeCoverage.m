function mergeCodeCoverage(modelName)
...
testDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'executable_tests', modelName, 'host');
covDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'code_coverages', modelName);
reqRslt = fullfile(testDir, 'high_level_tests', [modelName, '_SIL_REQ_Based_Test_Results.mldatx']);
sldvRslt = fullfile(testDir, 'low_level_tests', [modelName, '_SIL_SLDV_Based_Test_Results.mldatx']);
...
if ~exist(reqRslt, 'file')
    error(['Test results ''', modelName, '_SIL_REQ_Based_Test_Results.mldatx'' not found.']);
end
if ~exist(sldvRslt, 'file')
    error(['Test results ''', modelName, '_SIL_SLDV_Based_Test_Results.mldatx'' not found.']);
end
