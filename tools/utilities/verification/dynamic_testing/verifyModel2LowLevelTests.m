function verifyModel2LowLevelTests(modelName, varargin)
...
%   verifyModel2LowLevelTests(ModelName, AuthorNames)
...
resultFile = fullfile(fileparts(which(modelName)), '..', 'verification', 'simulation_tests', 'low_level_tests', [modelName, '_SLDV_Based_Test_Results.mldatx']);
if exist(resultFile, 'file')
    delete(resultFile);
end
rptFile = fullfile(fileparts(which(modelName)), '..', 'verification', 'simulation_tests', 'low_level_tests', [modelName, '_SLDV_Based_Test_Report.pdf']);
if exist(rptFile, 'file')
    delete(rptFile);
end
docDir = fullfile(fileparts(which(modelName)), '..', 'verification', 'simulation_tests', 'low_level_tests', [modelName, '_SLDV_Based_Test_Report']);
if exist(docDir, 'dir')
    rmdir(docDir, 's');
end
cvtFile = fullfile(fileparts(which(modelName)), '..', 'verification', 'model_coverages', 'low_level_tests', [modelName, '_SLDV_Based_Model_Coverage.cvt']);
if exist(cvtFile, 'file')
    delete(cvtFile);
end
htmlFile = fullfile(fileparts(which(modelName)), '..', 'verification', 'model_coverages', 'low_level_tests', [modelName, '_SLDV_Based_Model_Coverage_Report.html']);
if exist(htmlFile, 'file')
    delete(htmlFile);
end
gifDir = fullfile(fileparts(which(modelName)), '..', 'verification', 'model_coverages', 'low_level_tests', 'scv_images');
if exist(gifDir, 'dir')
    rmdir(gifDir, 's');
end
